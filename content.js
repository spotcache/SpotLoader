...
chrome.runtime.onMessage.addListener(async (message, sender, sendResponse) => {
    if (message.action === "getSongInfo") {
      try {
        // Fetch song name and artist from Spotify
        const songName = document.querySelector("div[data-testid='track-info-name']").innerText;
        const songArtist = document.querySelector("div[data-testid='track-info-artists']").innerText;
  
        // Generate the song download URL (replace with actual API if necessary)
        const songUrl = `https://some-api-or-source-to-download-song/${encodeURIComponent(songName)}.mp3`;
        ...
        sendResponse({ error: "Unable to fetch song information." });
      }
    }
  });
  ...
  // Fetch artist name from Spotify
